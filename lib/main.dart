...
class _AuthScreenState extends State<AuthScreen> {
  ...
  Future<void> _signInWithEmail() async {
    try {
      ...
      _showThankYouScreen(); // Navigate to the thank you screen on successful sign-in
    } catch (error) {
      setState(() {
        _errorMessage = error.toString();
      });
      ...
      body: LayoutBuilder(
        builder: (context, constraints) {
          return Row(
            children: [
              Expanded(
                flex: constraints.maxWidth > 1000 ? 1 : 0,
                child: Container(
                  color: const Color(0xFF121212),
                ),
              ),
              Expanded(
                flex: constraints.maxWidth > 1000 ? 1 : 2,
                child: Center(
...
class ThankYouScreen extends StatelessWidget {
  final FirebaseAuth auth; // Pass the auth instance to this screen
      ...
      body: Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 24),
            ElevatedButton(
              onPressed: () => _signOut(context), // Sign out and return to login
